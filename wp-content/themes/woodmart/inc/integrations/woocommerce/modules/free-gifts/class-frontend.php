...
class Frontend extends Singleton {
	...
	public function init() {
		...
		add_action( 'wp', array( $this, 'add_output_free_gifts_table' ), 100 );
	 ...
	 * @return void
	 */
	public function add_output_free_gifts_table() {
		...
		add_action( woodmart_get_opt( 'free_gifts_table_location', 'woocommerce_after_cart_table' ), array( $this, 'render_free_gifts_table' ) );
	}
	...
	public function update_gifts_table() {
		...
		if ( ! $table_html ) {
			$table_html = '<div class="wd-fg"></div>'; // Send empty wrapper to know where to add a table in the future.
		}
	 ...
	 *
	 * @param string $wrapper_classes String with css classes for table wrapper.
	 *
	 * @return void
	 */
	public function render_free_gifts_table( $wrapper_classes = '' ) {
	...
	public function update_cart_item_quantity( $cart_item_key, $quantity, $old_quantity, $cart ) {
		if ( ! isset( $cart->cart_contents[ $cart_item_key ]['wd_is_free_gift'] ) ) { //phpcs:ignore
			return;
		}

		if ( $quantity > 1 ) {
			if ( ! isset( $cart->cart_contents[ $cart_item_key ]['wd_is_free_gift_automatic'] ) && ! wc_has_notice( $this->manager->get_notices( 'already_added' ) ) ) {
